...
**Stage 7:**
> Light generalisation test. Pick one different Amsterdam-area zoning plan from ruimtelijkeplannen.nl, save its PDFs to data/inputs/synthetic_test/, run the pipeline. For every failure mode found, classify as: schema gap, hardcoded assumption, prompt brittleness, or API failure. Document findings in docs/generalisation_test.md briefly. The scope is intentionally lighter than originally planned because Stage 4b (IMRO cross-validation) provides an automatic generalisation signal on any Dutch project with a plan ID. Then write docs/written_response.md answering questions 4.1 through 4.4 from the brief, using the working notes in PROJECT_PLAN.md sections 11 and 12.

**Stage 7b:**
> Build the literal Scenario 1 demonstration. Create `data/inputs/draka_corrupted/` by copying `data/inputs/draka/` and editing the regels PDF to introduce one deliberate, plausible error in a binding height clause. Choose a single number to corrupt (for example, change a "21" to a "31" in one specific clause; document the exact location). The corruption must be plausible: in-range, syntactically valid, hard to spot by eye. Document the exact edit in `docs/corruption_demo.md` along with the page and source clause.
>
> Run the full pipeline on `data/inputs/draka_corrupted/`. In `docs/corruption_demo.md`, write up which layers of the Scenario 1 defence caught the error, with screenshots from the Streamlit viewer:
> - Did dual-pass extraction (Layer 3) catch it? Often the corrupted number does not appear elsewhere in the document, so the second pass returns the un-corrupted value from another location.
> - Did cross-document validation (Layer 4) catch it? Usually the toelichting references the same number in a separate context.
> - Did IMRO API cross-validation (Layer 4b) catch it? Almost certainly: the API holds the canonical uncorrupted plan.
> - Did sanity bounds (Layer 5) catch it? Probably not; the corruption stays within physical sense.
>
> The output is a short report with the corrupted field highlighted in the viewer next to its authoritative IMRO API value. This is the concrete, demonstrable answer to the Scenario 1 question in the brief.
