...
def main():
    ...
    print(f"\nWritten to {OUTPUT}")
    print("\nGrasshopper usage:")
    print("  Heron plugin  -> GH_GeoJSON component -> filter by object_type")
    print("  GH Python     -> import json; data = json.loads(File.ReadAllText(path))")
    print(
        "  Filter bouwvlakken: [f for f in data['features'] if f['properties']['object_type']=='bouwvlak']"
    )
